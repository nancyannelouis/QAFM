...
from config import (
    ...
    DATASET_CFG, QAFM_CFG, EVAL_Q_VALUES, ABLATION_Q_TRAIN_VALUES, NUM_WORKERS,
)
...
def train_and_eval_qtrain(
    q_train: int, k: int, dataset_name: str, device: str, epochs: int = 200
) -> dict:
...
def main():
    ...
    results = {}
    for qt in args.q_trains:
        results[qt] = train_and_eval_qtrain(qt, args.k, args.dataset, device, args.epochs)

    # 저장
    out_dir = os.path.join(RESULTS_DIR, "ablation")
    os.makedirs(out_dir, exist_ok=True)
    json_path = os.path.join(out_dir, f"{args.dataset}_qtrain_ablation.json")
    ...
    plot_asr_vs_quality(
        ...
        save_path=os.path.join(out_dir, f"{args.dataset}_qtrain_asr.png"),
        title=f"ASR vs Q_eval by q_train (k={args.k}, {args.dataset})"
    )
