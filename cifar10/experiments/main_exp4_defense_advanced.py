...
def run_fine_pruning(model, repair_loader, clean_loader, poison_loader, target_label, device,
    ...
    print(f"  가지치기만: BA={ba_prune_only:.2f}%, ASR={asr_prune_only:.2f}%")
    ...
    print(f"  가지치기+파인튜닝: BA={ba:.2f}%, ASR={asr:.2f}%")
...
def run_nad(model, repair_loader, clean_loader, poison_loader, target_label, device):
...
def main():
    ...
    for method_name in args.methods:
        ...
        print(f"  방어 적용 전: BA={ba_before:.2f}%, ASR={asr_before:.2f}%")

        fp_result  = run_fine_pruning(
            model, repair_loader, clean_loader, poison_loader, args.target_label, device,
            layer_name=args.prune_layer, prune_rate=args.prune_rate
        )
        nad_result = run_nad(model, repair_loader, clean_loader, poison_loader, args.target_label, device)
        sp_result  = run_shrinkpad(model, clean_loader, poison_loader, args.target_label, device)
    ...
    print(f"{'Method':<10} {'ASR전':>8} {'FP_ASR':>9} {'FP우회':>8} {'NAD_ASR':>10} {'NAD우회':>9} {'SP_ASR':>9} {'SP우회':>8}")
