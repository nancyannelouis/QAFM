"""
학습 스크립트 — 백도어 공격 모델 학습.

Usage:
    python train.py --dataset cifar10 --method qafm
    python train.py --dataset gtsrb   --method badnets
    python train.py --dataset cifar10 --method clean   # 클린 학습 (기준선)
"""
...
from datasets.poisoned_dataset import build_dataloaders, get_transforms, load_raw_dataset, PoisonedImageDataset
...
def main():
    ...
    print(f"[Train] dataset={args.dataset}, method={args.method}, "
    ...
    train_tf = get_transforms(args.dataset, train=True)
    ...
    from datasets.poisoned_dataset import EvalCleanDataset
    ...
    ckpt_name = f"{args.dataset}_{args.method}{tag}.pth"
    ...
    # ─── Save history ─────────────────────────────────────────────────────
    hist_path = os.path.join(RESULTS_DIR, f"train_history_{args.dataset}_{args.method}{tag}.json")
