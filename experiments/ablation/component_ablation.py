...
from config import (
    ...
    DATASET_CFG, TRAIN_CFG, QAFM_CFG, NUM_WORKERS,
)
from models  import build_model
from attacks import build_attack, QAFM, FTrojan
...
class QAFMNoJPEG(QAFM):
    ...
    def poison_image(self, image_np: np.ndarray) -> np.ndarray:
        ...
        from utils.jpeg_utils import rgb_to_ycbcr, ycbcr_to_rgb
...
def train_and_eval_asr50(attack, variant_name, dataset_name, device, epochs=200):
    """학습 후 ASR@Q50만 측정."""
    ...
    return {"BA": round(ba, 2), "ASR@Q50": round(asr, 2)}
...
def main():
    ...
    variants = {
        ...
        "Fixed-Delta": FTrojan(trigger_pos=(0,1), delta=20.0, target_label=0,
    ...
    for name, attack in variants.items():
        results[name] = train_and_eval_asr50(
            attack, name, args.dataset, device, args.epochs
        )

    # 저장
    out_dir = os.path.join(RESULTS_DIR, "ablation")
    os.makedirs(out_dir, exist_ok=True)
    json_path = os.path.join(out_dir, f"{args.dataset}_component_ablation.json")
    ...
    for name, res in results.items():
        print(f"{name:<15} {res['BA']:>7.1f}% {res['ASR@Q50']:>9.1f}%")
