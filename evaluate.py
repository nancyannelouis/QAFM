"""
평가 스크립트 — 학습된 백도어 모델의 BA / ASR @ 각 Q값 측정.

Usage:
    python evaluate.py --dataset cifar10 --method qafm --ckpt checkpoints/cifar10_qafm.pth
"""
...
def evaluate_all_q(
    model, attack, dataset_name: str, q_values: list,
    ...
    """
    _, _, test_imgs, test_lbls = load_raw_dataset(dataset_name, DATA_DIR)
...
def main():
    ...
    # ─── Model ───────────────────────────────────────────────────────────
    model = load_model(args.ckpt, ds_cfg["backbone"], ds_cfg["num_classes"], device)
    ...
    if attack is None:
        ...
        results = evaluate_all_q(
            model, attack, args.dataset, q_values,
    ...
    out_path = os.path.join(
        RESULTS_DIR,
        f"eval_{args.dataset}_{args.method}.json"
    )
