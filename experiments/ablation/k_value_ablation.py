...
from config import (
    ...
    DATASET_CFG, QAFM_CFG, ABLATION_K_VALUES, NUM_WORKERS,
    PSNR_THRESHOLD,
)
...
def train_and_eval_k(k: int, dataset_name: str, device: str, epochs: int = 200) -> dict:
...
def main():
    ...
    for k in args.k_values:
        results[k] = train_and_eval_k(k, args.dataset, device, args.epochs)

    # 저장
    out_dir = os.path.join(RESULTS_DIR, "ablation")
    os.makedirs(out_dir, exist_ok=True)
    json_path = os.path.join(out_dir, f"{args.dataset}_k_ablation.json")
    ...
    plot_ablation_k(
        ...
        save_path=os.path.join(out_dir, f"{args.dataset}_k_ablation.png")
    )
