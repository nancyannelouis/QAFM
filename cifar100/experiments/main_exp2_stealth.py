...
def evaluate_stealth(
    ...
    psnr_vals, psnr_trig_vals = [], []
    ssim_vals, ssim_trig_vals = [], []
    lpips_vals, lpips_trig_vals = [], []
    ...
    for i, idx in enumerate(tqdm(idxs, desc=f"[Exp2] {method_name}")):
        ...
        ssim_trig_vals.append(ssim(baseline, poisoned))
        lpips_vals.append(lpips_fn(clean, poisoned))
        lpips_trig_vals.append(lpips_fn(baseline, poisoned))
    ...
    return {
        ...
        "SSIM_trigger_only":  round(float(np.mean(ssim_trig_vals)),  4),
        "LPIPS":              round(float(np.mean(lpips_vals)),      4),
        "LPIPS_trigger_only": round(float(np.mean(lpips_trig_vals)), 4),
...
def check_thresholds(result: dict) -> dict:
    """
    논문 기준 임계값 충족 여부 확인.
    PSNR/SSIM/LPIPS 전부 *_trigger_only(트리거 자체의 순수 기여도) 기준 — Step5의
    JPEG 재압축 손실은 트리거 가시성과 무관하므로 세 지표 모두에서 판정 시 제외.
    """
    return {
        "PSNR_ok":  result["PSNR_trigger_only"]  >= PSNR_THRESHOLD,
        "SSIM_ok":  result["SSIM_trigger_only"]  >= SSIM_THRESHOLD,
        "LPIPS_ok": result["LPIPS_trigger_only"] <= LPIPS_THRESHOLD,
    }
...
def main():
    ...
    for method_name in args.methods:
        ...
        print(f"[Exp2] {method_name}: PSNR(트리거기여)={result['PSNR_trigger_only']:.2f}dB "
              f"(절대={result['PSNR']:.2f}dB), "
              f"SSIM(트리거기여)={result['SSIM_trigger_only']:.4f} (절대={result['SSIM']:.4f}), "
              f"LPIPS(트리거기여)={result['LPIPS_trigger_only']:.4f} (절대={result['LPIPS']:.4f}) | "
    ...
    # 논문 표 2 형식 출력 (전부 절대 / 트리거기여 두 버전 — 판정은 트리거기여 기준)
    print("\n[Exp2] 표 2: 은닉성 지표 비교 (트리거기여 기준이 판정용)")
    print(f"{'Method':<10} {'PSNR_절대':>10} {'PSNR_트리거':>11} "
          f"{'SSIM_절대':>9} {'SSIM_트리거':>10} {'LPIPS_절대':>10} {'LPIPS_트리거':>11}")
    for meth, res in all_results.items():
        print(f"{meth:<10} {res['PSNR']:>10.2f} {res['PSNR_trigger_only']:>11.2f} "
              f"{res['SSIM']:>9.4f} {res['SSIM_trigger_only']:>10.4f} "
              f"{res['LPIPS']:>10.4f} {res['LPIPS_trigger_only']:>11.4f}")

    # 결과 테이블 이미지 저장
    headers = ["PSNR 트리거기여(dB) ↑", "SSIM 트리거기여 ↑", "LPIPS 트리거기여 ↓"]
    table_data = {
        m: [f"{r['PSNR_trigger_only']:.2f}", f"{r['SSIM_trigger_only']:.4f}", f"{r['LPIPS_trigger_only']:.4f}"]
