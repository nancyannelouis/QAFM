...
Usage:
    python experiments/main_exp3_defense.py --dataset cifar10 \\
        --ckpt checkpoints/exp1_cifar10_qafm.pth --method qafm
"""
...
def run_neural_cleanse(model, clean_loader, num_classes, device, out_dir, method_name):
...
def main():
    ...
    ds_cfg   = DATASET_CFG[args.dataset]
    out_dir  = os.path.join(RESULTS_DIR, "exp3_defense")
    os.makedirs(out_dir, exist_ok=True)

    # ─── 공통 데이터 ─────────────────────────────────────────────────────
    _, _, test_imgs, test_lbls = load_raw_dataset(args.dataset, DATA_DIR)
    test_tf = get_transforms(args.dataset, train=False)
    ...
    for method_name in args.methods:
        ...
        ckpt_path = os.path.join(args.ckpt_dir, f"exp1_{args.dataset}_{method_name}.pth")
        if os.path.exists(ckpt_path):
            model = load_model(ckpt_path, ds_cfg["backbone"], ds_cfg["num_classes"], device)
        ...
        nc_result = run_neural_cleanse(
            model, clean_loader, ds_cfg["num_classes"], device,
        ...
        # 방법별 결과 저장
        method_result = {
            "neural_cleanse": nc_result,
            "strip": strip_result,
            "spectral_signatures": ss_result,
        }
        for key in ["clean_entropies", "poison_entropies", "scores", "anomaly_index", "l1_norms"]:
            if key in method_result.get("neural_cleanse", {}):
                pass
        with open(os.path.join(method_dir, "defense_results.json"), "w") as f:
            json.dump(all_summary[method_name], f, indent=2)

    # ─── 전체 요약 저장 & 출력 ───────────────────────────────────────────
    json_path = os.path.join(out_dir, f"{args.dataset}_defense_summary.json")
    ...
    make_result_table(
        table_data, headers,
        save_path=os.path.join(out_dir, f"{args.dataset}_table3_defense.png"),
